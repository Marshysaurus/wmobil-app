...
import 'package:wmobil/widgets/cards/ReportsCards/AssistsCards/AssistCard.dart';
import 'package:wmobil/widgets/cards/RestaurantPreviewCard.dart';

class AssistsScreen extends StatefulWidget {
  final String restaurantName;
  AssistsScreen({this.restaurantName});

  @override
  _AssistsScreenState createState() => _AssistsScreenState();
}
...
class _AssistsScreenState extends State<AssistsScreen> {
  ...
  List<pw.Widget> pdfWidgets = [
    pw.Header(level: 0, child: pw.Text('Asistencias'))
  ];
    ...
    if (picked != null) {
      if (picked.length == 1) {
        ...
        initialDateQuery = '${picked[0]}'
            ...
            .substring(0, 10);
        endDateQuery = '${picked[0]}'
            .replaceAllMapped('-', (match) => '/')
            .substring(0, 10);
        assists.fetchAssists(initialDateQuery + " " + initialHourForQuery,
      ...
      pw.Table(
        ...
        children: [
          pw.TableRow(children: [
            ...
            pw.Center(
                child: pw.Text(
              dateFormat.format(DateTime.parse(infoCard["entrada"])),
              style: pwBlackColumnBody500,
            )),
            pw.Center(
                child: pw.Text(
              dateFormat.format(DateTime.parse(infoCard["salida"])),
              style: pwBlackColumnBody500,
            )),
          ]),
        ],
                  ...
                  if (snapshot?.data["reporte"] == null)
                    return Container(
                      margin: EdgeInsets.only(
                          top: MediaQuery.of(context).size.height * .1),
                      child:
                          Text(snapshot.data["feedback"], style: wTextStyle500),
                    );

                  rows = snapshot.data["reporte"]["asistencias"].map((row) {
                    return row;
                  }).toList();
