...
class _AccountsScreenState extends State<AccountsScreen> {
  ...
  void showRangePicker() async {
    ...
    if (picked != null) {
      if (picked.length == 1) {
        ...
        discountAccounts.fetchDiscountsAccounts(
  ...
  void writeOnPdf(var infoCard) async {
    pdfWidgets.add(pw.Table(columnWidths: {
      ...
      pw.TableRow(children: [
        ...
        pw.Center(
            child: pw.Text(
          (infoCard["importe_descuento"] as num).toStringAsFixed(2),
          style: pwBlackColumnBody500,
        )),
      ]),
    ]));
  ...
  void writeTotalOnPdf({
    num totalizadorDeImportes,
    num totalizadorDeDescuentos,
  }) {
        ...
        children: [
          pw.TableRow(
            children: [
              pw.Center(
                child: pw.Text(
                  "\$" + totalizadorDeImportes.toStringAsFixed(2),
                  style: pwBlackColumnBody500,
                ),
              ),
              pw.Center(
                child: pw.Text(
                  "\$" + totalizadorDeDescuentos.toStringAsFixed(2),
                  style: pwBlackColumnBody500,
                ),
              ),
            ],
        ...
        SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              SizedBox(height: 8),
              StreamBuilder(
                  stream: discountAccounts.stream$,
                  builder: (context, snapshot) {
                    ...
                    if (cuentas.isEmpty) {
                      ...
                      writeTotalOnPdf(
                        totalizadorDeImportes: snapshot.data["totalizadorDeImporte"],
                        totalizadorDeDescuentos: snapshot.data["totalizadorDeDescuentos"],
                      );

                      return HighlightedPreviewCard(
                        cardData: snapshot.data["totalizadorDeDescuentos"],
